...
function Home() {
  ...
  useEffect(() => {
    // Check if user and currentchat exist before continuing
    if (user && currentchat) {
      const amigoId = currentchat?.members.find((m) => m !== user._id); // Find the amigoId

      if (amigoId) {
        // Proceed only if amigoId is found
        const getAmigodetails = async () => {
          try {
            console.log("userid " + user._id); // Debugging user ID
            const response = await axios.get(`${API_URL}api/users/${amigoId}`);
            setAmigo(response.data); // Set amigo details in state
          } catch (err) {
            console.error("Error fetching amigo details: ", err); // Error handling
          }
        };
          ...
          {currentchat ? (
            <>
              ...
              >
                ...
                <Picker data={data} onEmojiSelect={addEmoji} emojiSize={30} />
              </div>
